...
public class AirGetRequest extends AppCompatActivity {
    ...
    private void request() {
        ...
        // 步骤5:创建 网络请求接口 的实例
        final AirGetRequest_Interface request = retrofit.create(AirGetRequest_Interface.class);


        //对 发送请求 进行封装
        Call<List<AirBean>> call = request.getCall("air","1");
...
call.request().body();
                if(response.code() == 200){
                    ...
                    while (iterator.hasNext()){
                        ...
                        AirBean airBean = (AirBean)iterator.next();
                        airBean.show();
